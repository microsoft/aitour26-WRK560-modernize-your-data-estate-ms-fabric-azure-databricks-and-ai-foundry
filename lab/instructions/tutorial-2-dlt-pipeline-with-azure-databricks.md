...
2. Click on the **Sign in with Microsoft Entra ID**.
    ...
    ![Screenshot showing the sign in with Microsoft Entra ID option in Azure Databricks](/lab/instructions/media/databricks-signin.png)

3. On the left navigation pane, select **Jobs and pipelines**.

4. Select the **Create** dropdown and choose **ETL Pipeline**.

    ![Screenshot showing the create ETL pipeline option in Azure Databricks](/lab/instructions/media/databricks-createetl.png)
...
5. In the **Create ETL Pipeline** page, provide a name for your pipeline as `DLT_Pipeline`, scroll down to **Paths** and select the folder icon to browse the notebook.
    ...
    ![Screenshot showing the create ETL pipeline page in Azure Databricks](/lab/instructions/media/databricks-etl-details.png)
...
6. In the **Select file** dialog, select **Shared**, select **Analytics with ADB**, select **01 DLT Notebook** and then click on the **Select** button.
    ...
    ![Screenshot showing the selection of the 01 DLT Notebook in Azure Databricks](/lab/instructions/media/databricks-select-file.png)

7. In the **Destination** section, enter `dbo` as the **Default Schema** then select **Create**.

    ![Screenshot showing the destination selection for the pipeline in Azure Databricks](/lab/instructions/media/databricks-elt-destination.png)

8. Select **Start** to begin the pipeline execution.

    ![Screenshot showing the DLT pipeline start button in Azure Databricks](/lab/instructions/media/databricks-elt-pipeline-start.png)

Once the execution is completed, you will see a result similar to the following:

![Screenshot showing the DLT pipeline execution result in Azure Databricks](/lab/instructions/media/databricks-elt-result.png)
...
![Screenshot showing the DLT pipeline execution result in Azure Databricks](/lab/instructions/media/databricks-elt-lineage.png)
